...
| P1-15 | Settings shell: People, About, Set up this computer | done | (pending) | (pending) | `internal/settings` (People rename/remove, About, Setup with Chrome/Brave `--app=` commands built from `r.Host`). Fixed a P1-14 gap: nav "current" marking only matched exact paths, so `/settings/*` sub-pages never highlighted Settings — now prefix-aware. 8 Go unit tests, 5 settings E2E tests (rename, remove, About version, clipboard Copy, gate 1.43 `--app=` window), a11y/screens/no-side-scroll on all 3 settings pages — all green locally. |
