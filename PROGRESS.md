...
| P1-12 | Spike: offline test and upgrade/rollback harness | in-progress | (pending) | (pending) | Added `/__test/egress`, `deploy/test/offline.override.yaml`, `e2e/smoke/basic.spec.ts`, and the offline + upgrade/rollback sub-tests in `container-test.sh`. D-13 recorded. Pushing with [container] to exercise on Linux CI (untestable locally — no Docker engine on this machine). |
